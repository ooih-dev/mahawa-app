...
export default function ProfilePage() {
  ...
  const appItems = [
    ...
    },
    {
      ...
      right: <span className="text-xs text-[var(--color-text-muted)]">1.0.0</span>,
    },
  ];
    ...
    <div className="space-y-5">
      ...
      >
        <div className="flex items-center gap-2 mb-1">
          <ProfileIcon size={20} className="text-brand-500" />
      ...
      >
        <div className="px-5 py-4">
          ...
          {settingsItems.map((item, i) => (
            <SettingsRow key={i} icon={item.icon} label={item.label} desc={item.desc} right={item.right} />
          ))}
        </div>
      </div>
      ...
      >
        <div className="px-5 py-4">
          ...
          {appItems.map((item, i) => (
            <SettingsRow key={i} icon={item.icon} label={item.label} right={item.right} />
          ))}
        </div>
      </div>
